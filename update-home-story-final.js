async function updateHomeStory() {
  ...
  const updateUrl = `https://api.storyblok.com/v1/spaces/${SPACE_ID}/stories/${STORY_ID}`;
  const requestBody = {
    story: {
      content: content,
    },
  ...
  try {
    console.log('Updating home story (ID:', STORY_ID, ') in space', SPACE_ID, '...');
    ...
    if (response.ok) {
      ...
      console.log('\n✓ SUCCESS! Home story updated and published.');
      console.log('  Story ID:', data.story.id);
      console.log('  Slug:', data.story.slug);
      console.log('  Components:', data.story.content.body ? data.story.content.body.length : 0);
      console.log('  Published at:', data.story.published_at);
      console.log('\nNext steps:');
      console.log('  1. Update Netlify environment variable REACT_APP_STORYBLOK_TOKEN to: p5y8wD4f0W4g3qPr9uClpgtt');
      console.log('  2. Redeploy the React app on Netlify');
    } else {
